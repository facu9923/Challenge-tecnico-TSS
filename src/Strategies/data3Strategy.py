...
class data3Strategy(bt.Strategy):  
    ...
    def next(self):
        ...
        if price > self.sma10[0]:
            ...
            if size1 > 0:
                self.buy(data=self.datas[2], size=size1, info={strategy_id_sma10})
        elif price < self.sma10[0] and position.size > 0:
            for elemento in self.buy_records:
                if elemento['atributo'] == {strategy_id_sma10}:
                    self.sell(data=self.datas[2], size=elemento['clave'].pop(), info={strategy_id_sma10})
        ...
        if price > self.sma30[0]:
            size1 = cash_to_spend / price
            if size1 > 0:
                self.buy(data=self.datas[2], size=size1, info={strategy_id_sma30})
        elif price < self.sma30[0] and position.size > 0:
            for elemento in self.buy_records:
                if elemento['atributo'] == {strategy_id_sma30}:
                    self.sell(data=self.datas[2], size=elemento['clave'].pop(), info={strategy_id_sma30})
                    self.buy_records.remove(elemento) 

        strategy_id_crossover = 'crossover'
        if self.crossover > 0 and not position:
            size1 = cash_to_spend / price
            self.buy(data=self.datas[2], size=size1, info={strategy_id_crossover})
        elif self.crossover < 0 and position.size > 0:
            for elemento in self.buy_records:
                if elemento['atributo'] == {strategy_id_crossover}: 
                    self.sell(data=self.datas[2], size=elemento['clave'].pop(), info={strategy_id_crossover})
                    self.buy_records.remove(elemento) 