...
class data2Strategy(bt.Strategy):  
    ...
    def next(self):
        ...
        if price > self.sma10[0]:
            if int(size1) > 0:
                self.buy(data=self.data1, size=int(size1), info={strategy_id_sma10})
        elif price < self.sma10[0] and int(position.size) > 0:
            for elemento in self.buy_records:
                if elemento['atributo'] == {strategy_id_sma10}:
                    self.sell(data=self.data1, size = int(elemento['clave'].pop()), info={strategy_id_sma10})
        ...
        if price > self.sma30[0]:
            if int(size1) > 0:
                self.buy(data=self.data1, size=int(size1), info={strategy_id_sma30})
        elif price < self.sma30[0] and int(position.size) > 0:
            for elemento in self.buy_records:
                if elemento['atributo'] == {strategy_id_sma30}:
                    self.sell(data=self.data1, size = int(elemento['clave'].pop()), info={strategy_id_sma30})
                    self.buy_records.remove(elemento) 
        ...
        if self.crossover > 0 :
            if int(size1) > 0:
                self.buy(data=self.data1, size=int(size1), info={strategy_id_crossover})
        elif self.crossover < 0 and int(position.size) > 0:
            for elemento in self.buy_records:
                if elemento['atributo'] == {strategy_id_crossover}:
                    self.sell(data=self.data1, size = int(elemento['clave'].pop()), info={strategy_id_crossover})
                    self.buy_records.remove(elemento) 